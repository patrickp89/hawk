	...
	if (stat(HAWK_PROCFS_ENTRY, &pe_stat) == 0)
	{
		...
		if (f)
		{
			...
			while (k == 0)
			{
				switch (getch())
				{
				case GUI_ROTATE_LEFT:
					k = write_to_procfile(CONST_ROTATE_LEFT, f);
					break;
				case GUI_ROTATE_RIGHT:
					k = write_to_procfile(CONST_ROTATE_RIGHT, f);
					break;
				case GUI_AIM_UP:
					k = write_to_procfile(CONST_AIM_UP, f);
					break;
				case GUI_AIM_DOWN:
					k = write_to_procfile(CONST_AIM_DOWN, f);
					break;
				case GUI_FIRE_MISSILES:
					k = write_to_procfile(CONST_FIRE_MISSILES, f);
					break;
				case GUI_STOP_MOVEMENT:
					k = write_to_procfile(CONST_STOP_MOVEMENT, f);
					break;
				case GUI_QUIT_PROGRAM:
					k = 1;
					break;
				}
			}
